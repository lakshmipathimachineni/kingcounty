...
def read_stops(trip_ids):
    # Split the original string at each comma
    split_values = trip_ids.split(',')

    # Enclose each split value in single quotes
    quoted_values = ["'{}'".format(value) for value in split_values]

    # Join the quoted values using commas
    result_string = ','.join(quoted_values)
    ...
    cursor.execute(" select a.trip_id, b.stop_id,b.stop_code,b.stop_name,b.stop_desc,b.stop_lat,b.stop_lon, b.zone_id,b.stop_url,b.location_type,b.parent_station,b.stop_timezone\
     from stop_times a , stops b where a.trip_id in ("+result_string+") and a.stop_id =  b.stop_id")
